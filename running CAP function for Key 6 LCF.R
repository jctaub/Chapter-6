...
#function to remove the NA and turn it into a variable that says 'missing
...
#caluculates the CAP score (previously called the PAA score) for a key of 6 wherein the non-matches 
#are excluded
...
#running the function for the CART data
cartk6_wo0<-paa.6.m10.wo0(LCF2014, cart_lcf1, cart_lcf2, cart_lcf3, cart_lcf4, cart_lcf5, cart_lcf6, cart_lcf7,
              cart_lcf8, cart_lcf9, cart_lcf10, 'gor', 'output_area', 'tenure', 'dwelling', 
              'internet', 'hhsize', 'econ_ref')


#calculates the CAP score for a key of 6 wherein the non-matches are coded as 0
...
cartk6_nm<-as.data.frame(paa.6.m10(LCF2014, cart_lcf1, cart_lcf2, cart_lcf3, cart_lcf4, cart_lcf5, cart_lcf6, cart_lcf7,
                              cart_lcf8, cart_lcf9, cart_lcf10, 'gor', 'output_area', 'tenure', 'dwelling', 
                              'internet', 'hhsize', 'econ_ref'))



#running regression models for DCAP scores when non matches are excluded

cartk6_wo0_sub<-subset(cartk6_wo0, cartk6_wo0$m!='orig')
cartk6_wo0_sub$m<-as.numeric(as.character(cartk6_wo0_sub$m))

reg_cartk6_wo0<-lm(paa~m, data = cartk6_wo0_sub)
summary(reg_cartk6_wo0)

cartk6_wo0_sub$m2<-cartk6_wo0_sub$m^2

reg_cartk6_wo02<-lm(paa~m+m2, data=cartk6_wo0_sub)
summary(reg_cartk6_wo02)


#running regression models for the DCAP scores when non-matches are coded as 0
cartk6_nm_sub<-subset(cartk6_nm, cartk6_nm$m!='orig')
cartk6_nm_sub$m<-as.numeric(as.character(cartk6_nm_sub$m))

reg_cartk6_nm<-lm(paa~m, data = cartk6_nm_sub)
summary(reg_cartk6_nm)


cartk6_nm_sub$m2<-cartk6_nm_sub$m^2
cartk6_nm_sub$m3<-cartk6_nm_sub$m^3

reg_cartk6_nm2<-lm(paa~m+m2, data=cartk6_nm_sub)
summary(reg_cartk6_nm2)

reg_cartk6_nm3<-lm(paa~m+m2+m3, data=cartk6_nm_sub)
summary(reg_cartk6_nm3)
