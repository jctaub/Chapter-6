#This code synthesizes the 2014 LCF


#reduce the LCF to just the desired variablesand set said variables to factors
vars<- c('Gorx', 'OAC1D', 'A121', 'A116', 'A172', 'A049', 'a160p', 'A054', 'A093')
LCF2014<- X2014_dvhh_ukanon[, vars]

#gor-12, oa-8, a121-8, a116-6,a172-2, a049-9, a160p-4, a054-6, a093-7

ordered_vars<- c('Gorx', 'A049', 'OAC1D', 'A121', 'A093', 'A116', 'A054', 'a160p', 'A172')

LCF2014<- X2014_dvhh_ukanon[, ordered_vars]

LCF2014$Gorx<- as.factor(LCF2014$Gorx)
LCF2014$OAC1D<- as.factor(LCF2014$OAC1D)
LCF2014$A121<- as.factor(LCF2014$A121)
LCF2014$A116<- as.factor(LCF2014$A116)
LCF2014$A172<- as.factor(LCF2014$A172)
LCF2014$A049<- as.factor(LCF2014$A049)
LCF2014$a160p<- as.factor(LCF2014$a160p)
LCF2014$A054<- as.factor(LCF2014$A054)
LCF2014$A093<- as.factor(LCF2014$A093)

LCF2014[LCF2014=='']<- NA

#find missing values- only oac1d



#upload synthpop and synthesize the dataset both by CART (synthpop deault)
# and synthesize parametrically
...
para_lcf<- syn(LCF2014, method = 'parametric', m=10, seed=05301991)



#The code is written with my name as a sub-in so that either dataset can
# be subbed into jen and go through

#jen<- as.data.frame(cart_lcf$syn) #change as needed

#jen<-as.data.frame(para_lcf$syn)

jen1<- jen[, ordered_vars]


#due to how synthpop does MI each MI set has a numeric suffix
...
#here I renamed the variables from the LCF to names that actually make sense
...
#change the jen datasets into uniquely named datasets
